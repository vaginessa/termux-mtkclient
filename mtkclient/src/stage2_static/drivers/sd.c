...
int msdc_pio_read(struct msdc_host *host, void *buf)
{
    ...
    while (1) {
        ...
        while (left) {
            ...
            if ((msdc_rxfifocnt() >= MSDC_FIFO_THD) && (left >=  MSDC_FIFO_THD)) {
                                      ...
                                      unsigned long       timeout)
{
    ...
    if (opcode == MMC_READ_MULTIPLE_BLOCK) {
    ...
    } else if ((opcode == MMC_READ_SINGLE_BLOCK) || (opcode == MMC_SEND_TUNING_BLOCK)) {
        rawcmd |= (1 << 11);
        ...
        if (cmd->data->blocks > 1)
			...
			if ((cmd->data->blocks * host->blksz) > 256)
				sdr_set_field(EMMC50_CFG0, MSDC_EMMC50_CFG_ENDBIT_CNT, (host->blksz + 17));
			else
				sdr_set_field(EMMC50_CFG0, MSDC_EMMC50_CFG_ENDBIT_CNT, 273);
		} else {
			sdr_set_field(EMMC50_CFG0, MSDC_EMMC50_CFG_ENDBIT_CNT, 273);
		}
